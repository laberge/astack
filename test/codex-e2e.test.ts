...
describeCodex('Codex E2E', () => {
  ...
  testIfSelected('codex-discover-skill', async () => {
    ...
    const skillDir = path.join(testWorktree, '.agents', 'skills', 'astack-review');
  ...
  // Validates that Codex can invoke the astack-review skill, run a diff-based
  ...
  testIfSelected('codex-review-findings', async () => {
    // Install astack-review skill and ask Codex to review the worktree
    const skillDir = path.join(testWorktree, '.agents', 'skills', 'astack-review');
