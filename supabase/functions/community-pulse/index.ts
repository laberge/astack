...
Deno.serve(async () => {
  ...
  try {
    ...
    const { data: crashes } = await supabase
      .from("crash_clusters")
      .select("error_class, astack_version, total_occurrences, identified_users")
      .limit(5);
    ...
    const { data: versionRows } = await supabase
      .from("telemetry_events")
      .select("astack_version")
      .eq("event_type", "skill_run")
      .gte("event_timestamp", weekAgo)
      .limit(1000);

    for (const row of versionRows ?? []) {
      if (row.astack_version) {
        versionCounts[row.astack_version] = (versionCounts[row.astack_version] ?? 0) + 1;
      }
    }
