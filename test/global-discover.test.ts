...
const scriptPath = join(import.meta.dir, "..", "bin", "astack-global-discover.ts");

describe("astack-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/astack-global-discover.ts");
    ...
    test("SSH and HTTPS for same repo normalize to same URL", () => {
      const ssh = normalizeRemoteUrl("git@github.com:garrytan/astack.git");
      const https = normalizeRemoteUrl("https://github.com/garrytan/astack.git");
      const httpsNoDotGit = normalizeRemoteUrl("https://github.com/garrytan/astack");
