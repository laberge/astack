...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'astack-status-pill';
  ...
  statusPill.innerHTML = `<span class="astack-pill-dot"></span> astack${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'astack-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'astack-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'astack-ref-panel';

  const header = document.createElement('div');
  header.className = 'astack-ref-panel-header';
  header.textContent = `astack refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'astack-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'astack-ref-panel-row';
    row.innerHTML = `<span class="astack-ref-panel-id">${ref.ref}</span> <span class="astack-ref-panel-role">${ref.role}</span> <span class="astack-ref-panel-name">"${ref.name}"</span>`;
    list.appendChild(row);
  }
  if (refs.length > 30) {
    const more = document.createElement('div');
    more.className = 'astack-ref-panel-more';
