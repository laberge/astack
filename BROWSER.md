...
The window has a subtle green shimmer line at the top edge and a floating "astack" pill in the bottom-right corner so you always know which Chrome window is being controlled.
...
# Click the astack icon in toolbar → Open Side Panel
```
...
```bash
bin/astack-extension    # opens chrome://extensions, copies path to clipboard
```
...
4. **Navigate to the extension folder:** Press **Cmd+Shift+G** in the file picker to open "Go to folder", then paste one of these paths:
   - Global install: `~/.claude/skills/astack/extension`
   - Dev/source: `<astack-repo>/extension`
...
5. **Pin it:** Click the puzzle piece icon (Extensions) in the toolbar → pin "astack browse"
6. **Set the port:** Click the astack icon → enter the port from `$B status` or `.astack/browse.json`
7. **Open Side Panel:** Click the astack icon → "Open Side Panel"
...
| **Connection pill** | Small "astack" pill in the bottom-right corner of every page when connected |
