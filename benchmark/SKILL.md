...
echo '{"skill":"benchmark","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.astack/analytics/skill-usage.jsonl 2>/dev/null || true
...
mkdir -p .astack/benchmark-reports
mkdir -p .astack/benchmark-reports/baselines
```
...
Write to `.astack/benchmark-reports/baselines/baseline.json`.
...
Write to `.astack/benchmark-reports/{date}-benchmark.md` and `.astack/benchmark-reports/{date}-benchmark.json`.
