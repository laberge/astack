// astack telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/astack-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.astack_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        astack_version: String(event.astack_version).slice(0, 20),
