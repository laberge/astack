...
**Eureka Moments (if logged):** Read `~/.astack/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```bash
eval "$(~/.claude/skills/astack/bin/astack-slug 2>/dev/null)"
cat ~/.astack/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
```bash
DISCOVER_BIN=""
[ -x ~/.claude/skills/astack/bin/astack-global-discover ] && DISCOVER_BIN=~/.claude/skills/astack/bin/astack-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x .claude/skills/astack/bin/astack-global-discover ] && DISCOVER_BIN=.claude/skills/astack/bin/astack-global-discover
[ -z "$DISCOVER_BIN" ] && which astack-global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which astack-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/astack-global-discover.ts ] && DISCOVER_BIN="bun run bin/astack-global-discover.ts"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the astack directory to compile it." and stop.

Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/astack-discover-stderr
```

Read the stderr output from `/tmp/astack-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not "feat: astack-global-discover" + "feat: /retro global template".
...
```bash
ls -t ~/.astack/retros/global-*.json 2>/dev/null | head -5
```
...
```bash
mkdir -p ~/.astack/retros
```

Determine the next sequence number for today:
```bash
today=$(date +%Y-%m-%d)
existing=$(ls ~/.astack/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.astack/retros/global-${today}-${next}.json`:
...
```json
{
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: astack (58%) | Streak: 52d"
}
```
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.astack/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
