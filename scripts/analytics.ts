#!/usr/bin/env bun
/**
 * analytics — CLI for viewing astack skill usage statistics.
 *
 * Reads ~/.astack/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.astack', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('astack skill usage analytics');
