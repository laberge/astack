...
When Claude controls your real browser via CDP (astack `$B connect`), you look at two
windows: **Conductor** (to see Claude's thinking) and **Chrome** (to see Claude's actions).

astack's Chrome extension Side Panel shows browse activity — every command, result,
...
A "Session" tab in the astack Chrome extension Side Panel that shows:
...
## What astack builds (extension side)
