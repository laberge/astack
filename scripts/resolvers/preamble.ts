...
function generateUpgradeCheck(ctx: TemplateContext): string {
  return `If \`PROACTIVE\` is \`"false"\`, do not proactively suggest astack skills AND do not
...
If A: run \`${ctx.paths.binDir}/astack-config set proactive true\`
If B: run \`${ctx.paths.binDir}/astack-config set proactive false\`
