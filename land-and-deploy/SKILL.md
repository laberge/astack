...
echo '{"skill":"land-and-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.astack/analytics/skill-usage.jsonl 2>/dev/null || true
...
if [ ! -f ~/.astack/projects/$SLUG/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat ~/.astack/projects/$SLUG/land-deploy-confirmed 2>/dev/null)
...
```bash
~/.claude/skills/astack/bin/astack-review-read 2>/dev/null
```
...
```bash
mkdir -p ~/.astack/projects/$SLUG
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(cat .github/workflows/*deploy* .github/workflows/*cd* 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > ~/.astack/projects/$SLUG/land-deploy-confirmed
```
Continue to Step 2.
...
```bash
cat ~/.claude/skills/astack/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
```bash
ls -t ~/.astack-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
ls -t ~/.astack-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
Then run `astack-diff-scope` to classify the changes:

```bash
eval $(~/.claude/skills/astack/bin/astack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
```bash
$B snapshot -i -a -o ".astack/deploy-reports/post-deploy.png"
```
...
Save report to `.astack/deploy-reports/{date}-pr{number}-deploy.md`.
