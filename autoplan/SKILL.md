...
source <(~/.claude/skills/astack/bin/astack-repo-mode 2>/dev/null) || true
...
echo '{"skill":"autoplan","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.astack/analytics/skill-usage.jsonl 2>/dev/null || true
# zsh-compatible: use find instead of glob to avoid NOMATCH error
for _PF in $(find ~/.astack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do [ -f "$_PF" ] && ~/.claude/skills/astack/bin/astack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true; break; done
```
...
> astack can proactively figure out when you might need a skill while you work —
...
If A: run `~/.claude/skills/astack/bin/astack-config set proactive true`
If B: run `~/.claude/skills/astack/bin/astack-config set proactive false`

Always run:
```bash
touch ~/.astack/.proactive-prompted
```
...
AI makes completeness near-free. Always recommend the complete option over shortcuts — the delta is minutes with CC+astack. A "lake" (100% coverage, all edge cases) is boilable; an "ocean" (full rewrite, multi-quarter migration) is not. Boil lakes, flag oceans.
...
Before building anything unfamiliar, **search first.** See `~/.claude/skills/astack/ETHOS.md`.
...
If `_CONTRIB` is `true`: you are in **contributor mode**. At the end of each major workflow step, rate your astack experience 0-10. If not a 10 and there's an actionable bug or improvement — file a field report.

**File only:** astack tooling bugs where the input was reasonable but astack failed. **Skip:** user app bugs, network errors, auth failures on user's site.

**To file:** write `~/.astack/contributor-logs/{slug}.md`:
```
# {Title}
...
`~/.claude/skills/astack/office-hours/SKILL.md`
...
echo "RESTORE_PATH=$HOME/.astack/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Discover design docs: `ls -t ~/.astack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
- `~/.claude/skills/astack/plan-ceo-review/SKILL.md`
- `~/.claude/skills/astack/plan-design-review/SKILL.md` (only if UI scope detected)
- `~/.claude/skills/astack/plan-eng-review/SKILL.md`
...
- Test plan: generate artifact at `~/.astack/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at ~/.astack/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
~/.claude/skills/astack/bin/astack-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

~/.claude/skills/astack/bin/astack-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
~/.claude/skills/astack/bin/astack-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
~/.claude/skills/astack/bin/astack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

~/.claude/skills/astack/bin/astack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
~/.claude/skills/astack/bin/astack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
