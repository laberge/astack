...
If `PROACTIVE` is `true` (default): suggest adjacent astack skills when relevant to the
...
- Upgrades → /astack-upgrade

If the user opts out of suggestions, run `astack-config set proactive false`.
If they opt back in, run `astack-config set proactive true`.
