...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root astack' });
  }
...
describe('gen-skill-docs', () => {
  ...
  test('generated SKILL.md contains contributor mode check', () => {
    ...
    expect(content).toContain('astack_contributor');
  ...
  test('generated SKILL.md contains telemetry line', () => {
    ...
    expect(content).toContain('~/.astack/analytics');
  });
  ...
  test('preamble .pending-* glob is zsh-safe (uses find, not shell glob)', () => {
    for (const skill of ALL_SKILLS) {
      ...
      expect(content).toContain("find ~/.astack/analytics -maxdepth 1 -name '.pending-*'");
    }
  });
...
describe('REVIEW_DASHBOARD resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`review dashboard appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('astack-review');
...
describe('PLAN_FILE_REVIEW_REPORT resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`plan file review report appears in ${skill} generated file`, () => {
      const content = fs.readFileSync(path.join(ROOT, skill, 'SKILL.md'), 'utf-8');
      expect(content).toContain('ASTACK REVIEW REPORT');
    });
  }
...
describe('Plan status footer in preamble', () => {
  test('preamble contains plan status footer', () => {
    ...
    expect(content).toContain('ASTACK REVIEW REPORT');
    expect(content).toContain('astack-review-read');
...
describe('CODEX_SECOND_OPINION resolver', () => {
  const content = fs.readFileSync(path.join(ROOT, 'office-hours', 'SKILL.md'), 'utf-8');
  const codexContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'astack-office-hours', 'SKILL.md'), 'utf-8');
  ...
  test('Codex host variant does NOT contain the Phase 3.5 resolver output', () => {
    ...
    expect(codexContent).not.toContain('astack-codex-oh-');
  });
});
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      skills.push({ dir: '.', codexName: 'astack' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue; // /codex is excluded from Codex output
      if (!fs.existsSync(path.join(ROOT, entry.name, 'SKILL.md.tmpl'))) continue;
      const codexName = entry.name.startsWith('astack-') ? entry.name : `astack-${entry.name}`;
  ...
  test('root astack bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "astack"');
    expect(content).toContain('Use $astack to locate the bundled astack skills.');
  });

  test('codexSkillName mapping: root is astack, others are astack-{dir}', () => {
    // Root → astack
    expect(fs.existsSync(path.join(AGENTS_DIR, 'astack', 'SKILL.md'))).toBe(true);
    // Subdirectories → astack-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'astack-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'astack-ship', 'SKILL.md'))).toBe(true);
    // astack-upgrade doesn't double-prefix
    expect(fs.existsSync(path.join(AGENTS_DIR, 'astack-upgrade', 'SKILL.md'))).toBe(true);
    // No double-prefix: astack-astack-upgrade must NOT exist
    expect(fs.existsSync(path.join(AGENTS_DIR, 'astack-astack-upgrade', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/codex skill excluded from Codex output', () => {
    ...
    expect(fs.existsSync(path.join(AGENTS_DIR, 'astack-codex'))).toBe(false);
  });

  test('Codex review step stripped from Codex-host ship and review', () => {
    const shipContent = fs.readFileSync(path.join(AGENTS_DIR, 'astack-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(AGENTS_DIR, 'astack-review', 'SKILL.md'), 'utf-8');
  ...
  test('multiline descriptions preserved in Codex output', () => {
    ...
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'astack-office-hours', 'SKILL.md'), 'utf-8');
    const fmEnd = content.indexOf('\n---', 4);
    const frontmatter = content.slice(4, fmEnd);
  ...
  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['astack-careful', 'astack-freeze', 'astack-guard'];
  ...
  test('Codex preamble resolves runtime assets from repo-local or global astack roots', () => {
    // Check a skill that has a preamble (review is a good candidate)
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'astack-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('ASTACK_ROOT');
    expect(content).toContain('$_ROOT/.agents/skills/astack');
    expect(content).toContain('$ASTACK_BIN/astack-config');
    expect(content).toContain('$ASTACK_ROOT/astack-upgrade/SKILL.md');
    expect(content).not.toContain('~/.codex/skills/astack/bin/astack-config get telemetry');
  });

  // ─── Path rewriting regression tests ─────────────────────────

  test('sidecar paths point to .agents/skills/astack/review/ (not astack-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review → .agents/skills/astack-review
    // but setup puts sidecars under .agents/skills/astack/review/. Must match setup layout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'astack-review', 'SKILL.md'), 'utf-8');
    // Correct: references to sidecar files use astack/review/ path
    expect(content).toContain('.agents/skills/astack/review/checklist.md');
    expect(content).toContain('.agents/skills/astack/review/design-checklist.md');
    // Wrong: must NOT reference astack-review/checklist.md (file doesn't exist there)
    expect(content).not.toContain('.agents/skills/astack-review/checklist.md');
    expect(content).not.toContain('.agents/skills/astack-review/design-checklist.md');
  });

  test('sidecar paths in ship skill point to astack/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'astack-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/astack/review/');
      expect(content).not.toContain('.agents/skills/astack-review/checklist');
    }
  });
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('.agents/skills/astack/review/greptile-triage.md');
      expect(content).not.toContain('.agents/skills/astack-review/greptile-triage');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/astack → $ASTACK_ROOT
    expect(content).not.toContain('~/.claude/skills/astack');
    expect(content).toContain('$ASTACK_ROOT');

    // Rule 2: .claude/skills/astack → .agents/skills/astack
    expect(content).not.toContain('.claude/skills/astack');

    // Rule 3: .claude/skills/review → .agents/skills/astack/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      expect(content).not.toContain('~/.claude/skills');
      expect(content).not.toContain('.claude/skills');
      if (content.includes('astack-config') || content.includes('astack-update-check') || content.includes('astack-telemetry-log')) {
        expect(content).toContain('$ASTACK_ROOT');
      }
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('astack-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output unchanged: review skill still uses .claude/skills/ paths', () => {
    ...
    expect(content).toContain('~/.claude/skills/astack');
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of ALL_SKILLS) {
      const content = fs.readFileSync(path.join(ROOT, skill.dir, 'SKILL.md'), 'utf-8');
      expect(content).not.toContain('~/.codex/');
      // astack-upgrade legitimately references .agents/skills for cross-platform detection
      if (skill.dir !== 'astack-upgrade') {
        expect(content).not.toContain('.agents/skills');
      }
    }
  });
  ...
  test('codex host produces empty outside voices in design-review', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'astack-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('Design Outside Voices');
  });

  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'astack-ship', 'SKILL.md'), 'utf-8');
...
describe('setup script validation', () => {
  ...
  test('Codex install uses link_codex_skill_dirs', () => {
    ...
    expect(codexSection).not.toContain('ln -snf "$ASTACK_DIR" "$CODEX_ASTACK"');
  });
  ...
  test('setup separates install path from source path for symlinked repo-local installs', () => {
    expect(setupContent).toContain('INSTALL_ASTACK_DIR=');
    expect(setupContent).toContain('SOURCE_ASTACK_DIR=');
    expect(setupContent).toContain('INSTALL_SKILLS_DIR=');
    expect(setupContent).toContain('CODEX_ASTACK="$INSTALL_ASTACK_DIR"');
    expect(setupContent).toContain('link_codex_skill_dirs "$SOURCE_ASTACK_DIR" "$CODEX_SKILLS"');
  });
  ...
  test('Codex installs always create sidecar runtime assets for the real skill target', () => {
    ...
    expect(setupContent).toContain('create_agents_sidecar "$SOURCE_ASTACK_DIR"');
  });
  ...
  test('link_codex_skill_dirs reads from .agents/skills/', () => {
    ...
    expect(fnBody).toContain('astack*');
  });

  test('link_claude_skill_dirs creates relative symlinks', () => {
    // Claude links should be relative: ln -snf "astack/skill_name"
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('ln -snf "astack/$skill_name"');
  });
  ...
  // T1: Sidecar skip guard — prevents .agents/skills/astack from being linked as a skill
  test('link_codex_skill_dirs skips the astack sidecar directory', () => {
    const fnStart = setupContent.indexOf('link_codex_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('done', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('[ "$skill_name" = "astack" ] && continue');
  });

  // T2: Dynamic $ASTACK_ROOT paths in generated Codex preambles
  test('generated Codex preambles use dynamic ASTACK_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'astack-ship');
    ...
    expect(content).toContain('ASTACK_ROOT=');
    expect(content).toContain('$ASTACK_BIN/');
  });
  ...
  test('setup supports --host kiro with install section and sed rewrites', () => {
    ...
    expect(setupContent).toContain('~/.kiro/skills/astack');
  });
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('astack/SKILL.md');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('astack-upgrade/SKILL.md');
    ...
    expect(fnBody).not.toContain('ln -snf "$astack_dir" "$codex_astack"');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/astack', () => {
    expect(setupContent).toContain('migrate_direct_codex_install');
    expect(setupContent).toContain('$HOME/.astack/repos/astack');
...
describe('telemetry', () => {
  test('generated SKILL.md contains telemetry start block', () => {
    ...
    expect(content).toContain('astack-config get telemetry');
  });

  test('generated SKILL.md contains telemetry opt-in prompt', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('.telemetry-prompted');
    expect(content).toContain('Help astack get better');
    expect(content).toContain('astack-config set telemetry community');
    expect(content).toContain('astack-config set telemetry anonymous');
    expect(content).toContain('astack-config set telemetry off');
  });

  test('generated SKILL.md contains telemetry epilogue', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('Telemetry (run last)');
    expect(content).toContain('astack-telemetry-log');
