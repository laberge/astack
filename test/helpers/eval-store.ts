 ...
 * ~/.astack/projects/$SLUG/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
const LEGACY_EVAL_DIR = path.join(os.homedir(), '.astack-dev', 'evals');

/**
 * Detect project-scoped eval dir via astack-slug.
 * Falls back to legacy ~/.astack-dev/evals/ if slug detection fails.
 */
export function getProjectEvalDir(): string {
  try {
    // Try repo-local astack-slug first, then global install
    const localSlug = spawnSync('bash', ['-c', '.claude/skills/astack/bin/astack-slug 2>/dev/null || ~/.claude/skills/astack/bin/astack-slug 2>/dev/null'], {
    ...
    if (output) {
      ...
      if (slugMatch && slugMatch[1]) {
        const dir = path.join(os.homedir(), '.astack', 'projects', slugMatch[1], 'evals');
