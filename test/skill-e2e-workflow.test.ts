...
describeIfSelected('Ship workflow E2E', ['ship-local-workflow'], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'astack-ship-remote-'));
    shipWorkDir = fs.mkdtempSync(path.join(os.tmpdir(), 'astack-ship-work-'));
...
describeIfSelected('astack-upgrade E2E', ['astack-upgrade-happy-path'], () => {
  ...
  testConcurrentIfSelected('astack-upgrade-happy-path', async () => {
    const mockAstack = path.join(upgradeDir, '.claude', 'skills', 'astack');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/astack — use that exact path.`,
    ...
    recordE2E(evalCollector, '/astack-upgrade happy path', 'astack-upgrade E2E', result, {
