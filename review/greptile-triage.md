...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/astack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.astack/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/astack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.astack/projects/$REMOTE_SLUG"
mkdir -p ~/.astack
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.astack/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.astack/greptile-history.md` (global aggregate)
